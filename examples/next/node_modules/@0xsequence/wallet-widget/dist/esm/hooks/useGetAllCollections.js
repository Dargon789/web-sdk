...
export const useGetAllCollections = ({ accountAddresses, chainIds, hideUnlistedTokens }) => {
    const { data: tokenBalancesData, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } = useGetTokenBalancesSummary({
        chainIds,
        filter: {
            accountAddresses,
    ...
    const { data: collectionsWithMetadata } = useGetMultipleContractsInfo(collections?.map(collection => ({
